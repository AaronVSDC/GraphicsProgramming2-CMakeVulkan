...
namespace cvr {
	...
	{
		m_VikingRoomModel	= std::make_unique<Model>("Models/viking_room.obj");
		...
		m_VertexBuffer		= std::make_unique<VertexBuffer>(m_Device.get(), m_VikingRoomModel->getVertices());
		m_IndexBuffer		= std::make_unique<IndexBuffer>(m_Device.get(), m_VikingRoomModel->getIndices());
